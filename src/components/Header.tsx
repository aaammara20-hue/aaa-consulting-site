      ...
      <div className="container-custom flex items-center justify-between">
        ...
        <a href="#accueil" className="flex items-center gap-3 group">
          <img
            ...
            className="h-10 w-auto transition-transform duration-2000 group-hover:scale-105"
          />
        ...
        <nav className="hidden md:flex items-center gap-8">
          {navItems.map((item) => (
            <a
              key={item.label}
              href={item.href}
              className="text-muted-foreground hover:text-foreground transition-colors duration-2000 text-sm font-medium hover-underline"
            >
              {item.label}
            </a>
          ))}
